...
function [pval, t_orig, clust_info, seed_state, est_alpha, mn_clust_mass]=clust_perm1_iceeg_count(data,chan_hood,n_perm,fwer,tail,thresh_p,verblevel,seed_state,freq_domain)
...
df=n_subs_all-1; %t-score degrees of freedom
if verblevel~=0,
...
for perm=1:n_perm
    ...
    sm=nansum(d_perm,3);
    mn=sm./n_subs_all;
    sm_sqrs=nansum(d_perm.^2,3)-(sm.^2)./n_subs_all;
...
if tail==0,
    ...
    for a=1:n_clust,
        ...
        clust_mass=size(t_orig(use_ids),1); 
...
for z=1:n_clust,
    use_ids=(clust_ids==z);
    use_mass=-size(data_t(use_ids),1);
